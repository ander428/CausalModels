...
#' @description `orm` builds a logistic regression with the target as the treatment variable
...
#'
#' @returns \code{orm} returns an object of \code{\link[base::class]{class} "orm"}
#'
#' The function \code{summary} can be used to obtain and print a summary of the underlying glm model.
#'
#' An object of class \code{"orm"} is a list containing the following:
#'
#' \tabular{ll}{
...
#'             data = nhefs.nmv)
#'
#' p.score <- orm(nhefs.nmv)
#' p.score

orm <- function(data, f = NA, contrasts = list(), simple = pkg.env$simple, ...) {
  check_init()
...
#' @export
predict.orm <- function(x, newdata=NULL) {
  if(is.null(newdata)) {
